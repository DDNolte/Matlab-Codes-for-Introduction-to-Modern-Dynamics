%coupleNEdriver.m
% Modified from coupleNdriver.m on 1/15/24 to do ensemble averaging
...
writeavi = 0;
example = 6;        % 1 = global     2 = distributed    3 = square lattice    4 = 1D lattice   5 = tree  6 = ER 7 = Scale-free
...
for eloop = 1:Nens
    ...
    for facloop = 1:Nfac
        ...
        %     fac = facoef*(16*facloop/(Nfac))*sto/mnomega;   % Normalize by number of links in summation in coupleN.m
        fac = facoef*(50/facloop)*(1/avgdegree)*sto/mnomega;
...
% Entrainment graph
fig = figure(7);
axis([0 max(facval) -1.2*width/2 1.2*width/2])
...
figure(10)
plot(xxx(2:Nfac+1),MF);



dw = std(width*2*(0.5-rand(1,1000)));
arg = dw./(xxx(2:Nfac+1)*avgdegree);
...
Responst = (1 - stdev/maxs)*(1-1/sqrt(N)) + 1/sqrt(N);
...
aa = clock;
ID = round(100*aa(6));
save(strcat('meanf',num2str(ID)),'avgdegree','arg','MF','mean_field','Responst', 'xxx','yyy','xra','yra')
